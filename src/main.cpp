...
int mereniZapisRGB(int akualniBarva)
{ //Funkce zmeri RGB a zapíše do pole
  unsigned long cas = 0;
  cas = millis();
  ...
  { //Pri zastavení tridicky (zapis barvy do pole colors)
    ...
    do{  //Čtení červené barvy, pomocí nastavní pinů S2 a S3 na LOW
      digitalWrite(S2, LOW);
      digitalWrite(S3, LOW);
...
void vypisBarvu()
{                    //Funkce kontroluje a vypise barvu
  ...
  do
  { //Začátek měření;
    //přičtení hodnoty na místo barvy v poli podle toho která barva se vyskytuje nejčastěji
    int color;
    ...
    if (color == 2)
      help[2]++;
  } while (millis() - time < 2500); //meření po dobu 2500 milisekund
...
void loop()
{
  ...
  { // Pri stopu
    ...
    //Při stisku Enter spusť funkci odpovídajícího ID
